...
# Initialize model without moving it to GPU
model = cnn_models.CustomCNN()  # Use CPU (no .cuda())
...
out = cv2.VideoWriter('../outputs/bsl.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 30, (frame_width,frame_height))
...
while(cap.isOpened()):
    ...
    image = torch.tensor(image, dtype=torch.float)  # Use CPU (no .cuda())
